 ...
 */
public class ConnectFourGUI extends JFrame implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == newGameButton)	{
			...
			//output an error and don't allow the game to start if p1 & p2 chose identical colors
				...
				topLabel.setText("Click any button in a column or use the 0-6 keys to place your disk in that column.");
...
//-------------------- PRIVATE METHODS --------------------
	...
	private void updateStuff(){
		...
		if(game.gameOver())	{
			...
			directionsLabel.setText("Player " + game.getCurrentPlayer() + "'s turn.");
	}

	@Override
	public void keyTyped(KeyEvent e) {
		if(!game.gameOver() && buttons[0][0].isEnabled()) {
			try {
				game.play(Integer.parseInt("" + e.getKeyChar()));
			}catch(Exception ignored){}
			if (e.getKeyChar() == 'r') {
				game.play(ConnectFourBot.randomMove((ConnectFour) game));
			}else if(e.getKeyChar() == 'm') {
				directionsLabel.setText("Bot is Calculating.");
				int depth = 8;
				if (depth > 8)
					update(this.getGraphics());
				game = ConnectFourBot.chooseBestMove((ConnectFour) game, depth);
			}
			updateStuff();
		}
